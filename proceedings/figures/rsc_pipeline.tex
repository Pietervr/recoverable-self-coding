...
\node[box, minimum width=2.1cm, minimum height=1.0cm] (src) at (1.15,5.6)
  {candidate\\commitments};
\draw[flow] (src.east) -- (4.5,5.6)
...
\node[test, minimum width=2.2cm, minimum height=1.0cm] (t1) at (10.7,5.6)
  {certified in time?\\$T\le\Delta t$};
\draw[flow] (7.7,5.6) -- (t1.west)
  node[pos=0.42, above=1pt] {$T=W+S$};
\node[test, minimum width=2.6cm, minimum height=1.0cm] (t2) at (14.0,5.6)
...
\node[box, draw=good, fill=good!8, minimum width=2.1cm, minimum height=0.8cm] (rec)
  at (17.25,5.6) {recoverable};
% arrows between the boxes, drawn after the boxes exist and from their real edges
\draw[flow] (t1.east) -- (t2.west) node[midway, above=1pt, text=good] {yes};
\draw[flow] (t2.east) -- (rec.west) node[midway, above=1pt, text=good] {yes};

% "no" branches: one outcome box, no crossings
\node[box, draw=bad, fill=bad!6, minimum width=5.7cm, minimum height=0.8cm] (nrec)
  at (12.45,3.75) {not recoverable};
\draw[flow] (t1.south) -- (t1.south |- nrec.north);
\node[text=bad, align=left, anchor=west] at (10.8,4.62) {no: committed\\[-1pt]uncertified};
\draw[flow] (t2.south) -- (t2.south |- nrec.north) node[midway, right=1pt, text=bad] {no};

% ------------------------------------------------------------ measurement lane
\node[lane, text=meas] at (3.85,2.55) {Measurement: from event counts to the margin};

\node[mbox, minimum width=3.4cm, minimum height=1.3cm] (log) at (1.9,1.5)
  {event log: arrival times,\\completion times, busy time};
\node[mbox, minimum width=4.5cm, minimum height=1.3cm] (est) at (6.75,1.5)
  {$\hat R_{\mathrm{self}}=$ arrivals $/$ observation time\\[1pt]
   $\hat C_{\mathrm{self}}=$ completions $/$ busy time};
\node[mbox, minimum width=3.3cm, minimum height=1.3cm] (mar) at (11.3,1.5)
  {$\mathrm{CR}=R_{\mathrm{self}}/C_{\mathrm{self}}$\\[1pt]
   $\mathcal{M}=C_{\mathrm{self}}-R_{\mathrm{self}}$};
\draw[flow] (log.east) -- (est.west);
\draw[flow] (est.east) -- (mar.west);
\draw[flow] (mar.east) -- (14.55,1.5);
\node[align=center, text=soft] at (13.75,2.0) {queue law\\[-1pt](Prop.~1)};

% what is observed (dashed; the two paths do not cross)
\draw[obs] (3.3,5.6) -- (3.3,3.35) -- (1.0,3.35) -- (1.0,0 |- log.north);
\draw[obs] (9.0,5.6) -- (9.0,3.0) -- (2.6,3.0) -- (2.6,0 |- log.north);
