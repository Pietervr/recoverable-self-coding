...
# by bench.py (2026-09-11: one dataset-layer of the full §8 procedure = 223 s on one core at D = 4, 8 starts
# everywhere; ~1.7x at D = 8). Two chains: A on 13 workers, B (the gain calibration) on one.
#
...
"$PY" simulate.py calibration --n-rep 60 --D 4 --layers 41 --n-jobs "$NJOBS" --seed 2026 > "$LOG/calibration_D4.log" 2>&1

echo "waiting for the gain calibration $(date)"
while [ ! -f "$OUT/gain_calibration.done" ]; do sleep 60; done

echo "power D4 $(date)"
"$PY" simulate.py power --n-rep 60 --D 4 --layers 41 --n-jobs "$NJOBS" --seed 2026 > "$LOG/power_D4.log" 2>&1

echo "recovery D4 $(date)"
"$PY" simulate.py recovery --n-rep 20 --D 4 --layers 41 --n-jobs "$NJOBS" --seed 2026 > "$LOG/recovery_D4.log" 2>&1

echo "power D8 $(date)"
"$PY" simulate.py power --n-rep 40 --D 8 --layers 41 --n-jobs "$NJOBS" --seed 2026 > "$LOG/power_D8.log" 2>&1

echo "calibration D8 $(date)"
"$PY" simulate.py calibration --n-rep 40 --D 8 --layers 41 --n-jobs "$NJOBS" --seed 2026 > "$LOG/calibration_D8.log" 2>&1

# the layer-averaging check: two nulls and two alternatives on the five-layer grid
echo "five-layer check $(date)"
"$PY" simulate.py calibration --n-rep 30 --D 4 --n-jobs "$NJOBS" --seed 2026 --generators M2B,M2K \
      --out "$OUT/calibration_D4_5layers.csv" > "$LOG/calibration_D4_5layers.log" 2>&1
"$PY" simulate.py power --n-rep 30 --D 4 --n-jobs "$NJOBS" --seed 2026 --generators M3H,M3V \
