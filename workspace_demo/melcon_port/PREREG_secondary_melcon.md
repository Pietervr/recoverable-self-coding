# DRAFT v6 — the planned secondary human analysis on Melcón et al. 2024 (OpenNeuro ds006171)

**Status: DRAFT v6 (2026-09-14), after Codex's reads of v1–v5
(`../t1_access/reviews/2026-09-13_cheap_cloud_runs_and_melcon_codex_record.md` §B, continuation reviews 2–5 and the v5
calibration revision record `../t1_access/reviews/2026-09-14_melcon_v5_calibration_revision_codex_record.md`, RSC
563d52a). The v5 strength calibration — outcome-blind, decoder AUC only, retained in `results/battery/v5-7fcb46729408/` —
found the eight-template statistic's spread between draws as large as its ±0.03 tolerance, three acceptances without a
grid crossing and two cells not usable, so stage C on v5 could not establish battery success. v6 revises only the
calibration and the battery's identity (§9): strength against an empirical reach on all 34 templates, independent seed
phases, an interior bracket with one bounded refinement and a new terminal check, complete statistics, unresolved-strength
flags, the corrected G2 integration, the renamed readout contrast, and the runtime, the BMS port and the result payloads
bound into the identity. The rest of v5 stands: per-generator strength, the label-free inclusion gate before any decoding,
complete cells and immutable result provenance, the component-gap diagnostics, the graded-spread wording and the sample
counts (§2, §6, §9, §11). Before the freeze remain: the battery's v6 calibration and stage C (§9); the causal-edge and
composite filter-support corrections (§3, §5); the §7 summary assembly and output command with the two-model BMS and
legacy sensitivity integrations (§6–§7); the both-tasks decoder sensitivity (§4); the report-conditioned description
(§10); the all-recording loader verification (§3); and Codex's read of the result.
...
*Decoder strength, per generator, calibrated independently of every family outcome (v6; Codex, v5 calibration revision record).* The calibration statistic is the median over recordings of each recording's mean held-out presence AUC over its two halves and the ten main-interval windows, computed on a draw of **all 34 templates** without drift. It is **complete** only when all 34 recordings decode and all 680 main-window AUCs are finite and in [0, 1]; an incomplete statistic is recorded with its failures and enters no reach, bracket or acceptance. **Reach.** For each generator, R_g is the mean of two independently seeded complete statistics at amplitude 6.4, saved with both values, their per-template AUCs and their disagreement, and frozen before either strength is searched. R_g is an empirical finite-amplitude reference: two draws neither establish an attainable ceiling nor estimate the noise SD, and R_g ≤ 0.5 or an incomplete reach draw makes that generator's calibrations not usable. **Targets** are 0.5 + q (R_g − 0.5): **weak** q = 0.5 and **strong** q = 0.8. (v5 took 0.5 and 0.9 of the latent-ranking limit on the first eight templates; its calibration grids levelled off below those limits, three strong or stress cells were accepted only at the grid edge, X1 strong and X2 weak were not usable, and the statistic differed by 0.025–0.068 between independent draws at the same amplitude. That record is kept, and its averaged draws are not read as proof that every v5 strong target was unattainable.) The **latent-ranking reference** — the AUC of ranking each present trial by its noiseless latent against catch (`synthetic.present_latent_auc`; exact for G1, G3, X1 and X2, for G2 a grid convolution with a trapezoidal CDF) — is reported beside R_g and never used as a target; it is the optimal population ROC only for X1 and X2 (G3 has unequal present and catch variances). On all 34 templates it is G1 0.778, G2 0.781, G3 0.725, X1 0.743 and X2 0.647. v5's G2 integration took a plain cumulative sum and overstated the first-eight-template value by 0.000196 (0.783929 against 0.783734, Codex, continuation review 5 §1); v6's trapezoid rule agrees with adaptive quadrature to 4e-8 and with Codex's fine-grid value to 7e-8 (`test_battery.py`). **Search.** One calibration draw per generator and strength, common across amplitudes, on the grid 0.2, 0.4, 0.7, 1.0, 1.5, 2.2, 3.2 and 6.4. The amplitude comes from the **interior bracket**: in ascending amplitude, the first adjacent pair with s(lo) < target ≤ s(hi) and 0.2 ≤ lo < hi < 6.4, interpolated linearly. There is no usable bracket if the statistic at 0.2 already meets the target, if the target is not reached below 6.4, or if a statistic at or before the crossing is incomplete; a later crossing of a non-monotone curve is never chosen, and non-monotone steps are flagged with the whole curve saved. With a bracket, an independent **check** draw at that amplitude is accepted within ±0.03 of the target. Otherwise one refinement adds the points 0.75, 0.875, 1, 1.125 and 1.25 times the amplitude (rounded to four decimals, clipped to [0.2, 6.4], an identical point reused) on the calibration draw, the bracket is re-read on the union by the same rule, and its amplitude receives a **new terminal check** draw, independent of the search and of the first check, which alone decides; the search then stops. At most 15 statistics are drawn per cell (8 grid, 5 refinement, 2 checks). No usable bracket, or a terminal check that is incomplete or outside the tolerance, makes the calibration **not usable**: it is never retargeted, widened or retried; its cells are diagnostics, run only when an amplitude exists, identified as such in every result and verdict, and they neither pass, fail nor drive a revision. **Tolerance.** ±0.03 is a declared development tolerance, not a confidence interval or a demonstrated precision, and moving from eight to 34 templates is not assumed to halve the spread. The weak and strong targets differ by 0.3 (R_g − 0.5); where that is at most 0.06 the two acceptance bands overlap and the strengths are **unresolved** at this tolerance, and a strong check at or below the weak check is a **reversal**. Both are flagged per generator (`strengths.json`, the stage C receipt) and neither counts as demonstrating two distinct strengths. X2 is the expected unresolved case: with a reach near 0.60 its weak target lies near 0.55, which is not §8's per-window sensitivity gate, and no gate moves to force a result. **Seeds**, by phase: reach (4, generator, draw, subject), calibration (5, generator, strength, subject), check (6, …), terminal check (7, …), stage C (1, generator, strength, drift, replicate, subject). The calibration is conditional on these 34 templates; transport to unseen templates is not tested. Equal presence AUC across generators is not equal mixture identifiability, so the readout diagnostics below are reported beside every cell.

*Result provenance and completeness.* A configuration identity names the result namespace `results/battery/v6-<digest>/`: the battery version and result schema; the SHA-256 of every covered module as imported and of the imported BMS port; every module's specification; the strength, grid, refinement, tolerance, statistic and seed-phase constants; the effective template list with each template's digest; the events table's digest; and the runtime — the Python, NumPy, SciPy, scikit-learn, pandas and joblib versions, the numerical thread variables and the effective thread pools. The identity is re-derived **in the executing process**, before every calibration statistic and every stage C recording and in parallel workers too, and a mismatch or a module changed on disk since import stops the run. Reach and calibration entries are sealed with a SHA-256 of their content, checked on every load, and an existing entry is never replaced. Every recording result carries its manifest (identity, generator, strength, drift, replicate, subject, seeds, amplitude, calibration entry hash and usability, template digest) and is written with a sidecar holding the payload's SHA-256, the manifest digest and the schema; reuse requires the sidecar, the payload hash, the manifest and the result schema, so a damaged, foreign or incomplete result is refused, and a revision writes a new namespace beside the retained old one. Stage C launches only through `stage_c_launch.py`, which also checks the reviewed environment (BMS 6d48a893…1717611, Python 3.14.6, NumPy 2.5.3, SciPy 1.18.1, scikit-learn 1.9.1), refuses uncommitted covered modules and writes a run receipt with the reach and calibration hashes and the strength-resolution flags. A cell is judged only when every replicate has every recording; otherwise it is **incomplete**, which is distinct from a failure. The final battery report summarizes the full generator set. The per-cell **unadjusted high/low readout contrast** (v5's 'readout component separation', renamed in v6) is pooled over a half's two blocks — not conditioned on dose, hemifield or block, and a different estimand from the analytic conditional component gap — and is reported as a descriptive diagnostic like the readout–latent correlation, never a pass criterion. The v5 namespace and its ten calibration entries are retained unchanged as the historical computation, including its G2 approximation.
...
*Cost, planning evidence.* One complete synthetic recording took 19.2 s of pipeline and 0.64 s of generation on one core of the Mac shared with the refit probe (`results/battery/benchmark.json`), projecting about 11.2 core-hours for the 2,040 recordings of stage C; group BMS, common-cohort reruns and the sensitivities are outside that figure. The v6 calibration is bounded at 5,440 generated decoder calls — reach 5 × 2 × 34 = 340, cells 10 × 9 × 34 = 3,060 and up to 10 × 15 × 34 = 5,100 — about 11.0–17.5 core-hours at an assumed 11.6 s per call (the benchmark does not time the decoder alone). Generation, decoder time and peak memory are measured on the first reach draw with one worker, and a second low-priority process is added only if that evidence supports it beside the probe's eleven workers. An extra grid point or all-cell check would add 340 calls and is costed before any extension. No cloud.
...
synthetic and artificial inputs only; stage C of the battery is projected at ≈ 11 core-hours plus ≈ 11–17.5 for the v6 strength calibration (planning evidence, §9) and is staged around the running refit probe. The full EEG processing runs on the PC after its
